...
0. **A BLOCKED JOB WHOSE GATE THE HUMAN HAS CLEARED COMES FIRST.** Call `jobs` action `list` with `namespace: <ns>` and `status: blocked`. **You do not resume your own blocked job with a plain resume: the Worker refuses it unless the caller is the admin or holds `can_merge`**, because a driver clearing its own gate is an approval nobody gave. If one is waiting AND the human has said in this conversation that they ran its command, ask them to resume it from the seat or the console, naming the job id; the resume hands the job back to you claimed, and step 1 picks it up. Blocked is a PAUSE, not an ending: the same job carries its own outcome, which is why `resume` exists. A blocked job nobody has spoken about stays blocked, and you say it is waiting. The one resume a driver may make on its own job is the policy resume in step 4b, for a branch push or a pull request. If an older Worker still accepts your plain resume, do not use it: the rule above is the one that holds.
...
2b. **READ THE OFFERED SKILLS.** A successful `claim` returns `offered_skills`: up to three skills the Worker matched to this job, each with its `trigger_condition`, its `recent_failures` and its full instructions in `body`. The Worker records the offer itself, so there is nothing to report about what was offered. Read each one. Follow a skill only where its trigger fits this job; one whose trigger does not fit is left alone, and that is a normal outcome, not a failure. Keep the ids of the skills you actually followed. The list is often empty. A job claimed again after a resume or a release returns the same offer it had the first time.
...
5. **Finish it, exactly once:**
   ...
   - **On `complete` or `fail`, report the skills you followed** as `skills: { used: [<ids>] }`, only ids from this job's `offered_skills`, and `used: []` when you followed none. Do not send `offered`: the Worker stores its own record, and refuses an offered list that differs from it or a used skill it did not offer. Name a skill as used only if it changed what you did. Offered-but-not-used is what the evaluation compares against.
...
3. For each namespace whose `paused` is `null` (skip every paused one by name in a one-line note; never touch a namespace not in the map above):
   ...
   3b. **THE PATH GUARD RUNS ON EVERY ATTEMPT, BEFORE ANY PUSH OR DISPATCH.** It is deterministic and it is not your judgement:
       ...
       node C:\Users\email\dev\capsid-mcp\scripts\path-guard.mjs protected.json <base> HEAD
       ```
